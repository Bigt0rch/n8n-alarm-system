...
Ejemplo:
  cat ~/.n8n-files/alarms.json | python alarmMonitorDBn8n.py \\
...
def escribir_servicio_gen(write_api, influx_org: str, influx_bucket: str,
    ...
    manteniendo el mismo esquema que usaba el plugin exec de Telegraf.
    """
    point = (
        Point("servicio_gen")
        .tag("nombre", nombre)
    ...
    log.debug(f"servicio_gen[{nombre}].status_code = {status_code} escrito en InfluxDB")
...
def procesar_alarma(alarma: dict, query_api, write_api,
    ...
    if categoria == "servicio":
        # Chequeo HTTP directo; también persiste el resultado en servicio_gen
        url          = alarma["url"]
        ...
        # Resto de categorías: leer de InfluxDB
    ...
    # 4. Escribir en InfluxDB y devolver cambio solo si hay transición
    ...
    for alarma in alarmas:
        try:
            resultado = procesar_alarma(
                alarma, query_api, write_api,
                influx_org, influx_bucket, log
            )
            if resultado:
                cambios.append(resultado)
        except Exception as e:
            log.error(f"Error procesando '{alarma['nombre']}': {e}", exc_info=True)
